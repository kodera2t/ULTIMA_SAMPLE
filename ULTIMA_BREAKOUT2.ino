//// Sample program "BREAKOUT" for ULTIMA and Zero Color2, and ULTRA Zero Color(need I2C soldering)
...
///wall-touch check
 ...
 //SerialUSB.println(error,DEC);
  ...
  oldy=ycell;
        ...
        oled.fillCircle(numx,numy,rad,BLUE);
        tmpx=numx;
        tmpy=numy;
        delay(500);
        oled.clearScreen();        
...
//// drawing block ////
        for( i=1;i<=blocknum;i++){
          if(firstline[i]==1){
          oled.drawRect((i-1)*blockw,fpos,blockw,blockh,WHITE);
          }
        }
        k=0;
        for( i=1;i<=blocknum;i++){
        ...
        oled.fillRect(padposi,padd-padh,padw,padd,RED);  
        oled.fillCircle(newx,newy,rad,BLUE);
        oled.setTextScale(1);
